...
new Text:CountLive[MAX_PLAYERS];//текст-дравы жизней игроков
new Text:Live[MAX_PLAYERS];
...
public OnFilterScriptInit()
{
	...
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
		...
		Live[i] = TextDrawCreate(510.000000,251.000000,"LIVE:");//текст-дравы жизней игроков
		TextDrawAlignment(Live[i],0);
		TextDrawBackgroundColor(Live[i],0x000000ff);
		TextDrawFont(Live[i],1);
		TextDrawLetterSize(Live[i],0.599999,2.599999);
		TextDrawColor(Live[i],0xff3333ff);
		TextDrawSetOutline(Live[i],1);
		TextDrawSetProportional(Live[i],1);
		TextDrawSetShadow(Live[i],1);

		CountLive[i] = TextDrawCreate(565.000000,250.000000,"_");
		TextDrawAlignment(CountLive[i],0);
		TextDrawBackgroundColor(CountLive[i],0x0000ff66);
		TextDrawFont(CountLive[i],2);
		TextDrawLetterSize(CountLive[i],0.699999,2.699999);
		TextDrawColor(CountLive[i],0xffffffff);
		TextDrawSetOutline(CountLive[i],1);
		TextDrawSetProportional(CountLive[i],1);
		TextDrawSetShadow(CountLive[i],1);
...
public OnFilterScriptExit()
{
	...
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
	    ...
	    TextDrawHideForPlayer(i, CountLive[i]);//текст-дравы жизней игроков
	    TextDrawHideForPlayer(i, Live[i]);
		TextDrawDestroy(CountLive[i]);
		TextDrawDestroy(Live[i]);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	TextDrawHideForPlayer(playerid, CountLive[playerid]);//текст-дравы жизней игроков
	TextDrawHideForPlayer(playerid, Live[playerid]);
...
//				printf("[Derby] Админ %s [%d] починил транспорт всем участникам дерби-сумо.", sendername, playerid, mpstate[playerid], mpvw[playerid]);
	...
	if(strcmp(cmd, "/dskick", true) == 0)
	{
...
#endif
		{
			...
			TextDrawHideForPlayer(para1, CountLive[para1]);//текст-дравы жизней игроков
			TextDrawHideForPlayer(para1, Live[para1]);
...
public dsCarSpawn(playerid, para1)//спавн транспорта
{
	...
	TextDrawSetString(Live[playerid], "LIVE:");
	TextDrawShowForPlayer(playerid, Live[playerid]);

	format(String, sizeof(String), "%d", dslive[playerid]);
	TextDrawSetString(CountLive[playerid], String);
	TextDrawShowForPlayer(playerid, CountLive[playerid]);
	...
	for(new i = 0; i < MAX_PLAYERS; i++)//цикл для всех игроков
	{
		if(IsPlayerConnected(i))//дальнейшее выполняем если игрок в коннекте
		{
			if(dsstate[i] != 0)
			{
				...
				TextDrawSetString(CountLive[i], string);
